...
function Device(app, opts) {
  ...
  this.D = 501; // 501 is a CPU sensor
  this.name = 'CPU@' + os.hostname() + ' (% use)';
  ...
  process.nextTick(function() {
    ...
    // Run Top for 3 cycles to eliminate the use of summary information
    var execcmd = 'top -b -n3 -d1 | grep \'Cpu(s)\' | tail -n+3 | awk \'{print $2}\' | cut -d\'%\' -f1'

    child = exec(execcmd,
    function (error, stdout, stderr) {
      stdout.replace(/(\n|\r|\r\n)$/, '');
      self._app.log.debug('[ninja-cpu] CPU is now at %s, updating every %s seconds', stdout, opts.poll_interval);
